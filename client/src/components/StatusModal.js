...
const StatusModal = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (status.onEdit) {
      dispatch(
        updatePost({ content, contentsub, community, images, images2, auth, status })
      );
      //added images2
    } else {
      dispatch(
        createPost({ content, contentsub, community, images, images2, auth, socket })
      );//added images2
    }
  ...
  useEffect(() => {
    if (status.onEdit) {
      ...
      setImages2(status.images2); //added part
    }
  }, [status]);
  console.log(images);
