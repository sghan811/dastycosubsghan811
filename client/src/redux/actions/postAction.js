...
export const updatePost = ({
  ...
  //added from here
  let media2 = [];
  const imgNewUrl2 = images2.filter((img) => !img.url);
  const imgOldUrl2 = images2.filter((img) => img.url);
  //end
  if (
  ...
  try {
    ...
    const res = await patchDataAPI(
      `post/${status._id}`,
      { content, contentsub, community, images: [...imgOldUrl, ...media], images2: [...imgOldUrl2, ...media2] },
      auth.token
    ); //added images2: [...imgOldUrl2, ...media2]
