...
const postCtrl = {
  createPost: async (req, res) => {
    try {
      const { content, contentsub, community, images, images2 /*added images2*/} = req.body;
  ...
  LikeleftUnLikerightPost: async (req, res) => {
    try {
      const post = await Posts.find({
        _id: req.params.id,
        likelefts: req.user._id,
      ...
      res.json({ msg: "Post unlikerightd successfully. and Post likeleftd successfully" });
    } catch (err){
      return res.status(500).json({ msg: err.message });
    }
  },  
  UnLikeleftLikerightPost: async (req, res) => {
    try {
      const post = await Posts.find({
        _id: req.params.id,
        likelefts: req.user._id,
      ...
      if (post.length > 0) {
        return res
          .status(400)
          .json({ msg: "You have already likerighttd this post" });
      }
